...
<div class="main">
	<div class="manage-sec-create">
		<form action="<%=path%>/season/updateSeason" method="post" id="updateSeasonForm">
			<h2>更新赛季配置</h2>
			<input type="hidden" name="originalSeasonId" value="${season.id}">
			<div class="form-item">
				<label for="seasonId">赛季ID</label>
				<input type="text" id="seasonId" name="updateSeasonId" readonly value="${season.id}" class="ui-input" placeholder="赛季ID，例：201807" autocomplete="off" required pattern="^[A-Za-z0-9]|[0-9]$">
			</div>
			<div class="form-item">
				<label for="seasonName">赛季名称</label>
				<input type="text" id="seasonName" name="seasonName" class="ui-input" value="${season.seasonName}" placeholder="赛季名称" autocomplete="off" required>
			</div>
			<div class="form-item">
				<label for="seasonDescription">赛季描述</label>
				<textarea name="seasonDescription" id="seasonDescription" cols="30" rows="5" required class="ui-textarea" placeholder="赛季简要描述">${season.seasonDescription}</textarea>
			</div>
			<div class="form-item">
				<label for="seasonKeyWords">赛季关键词</label>
				<textarea name="seasonKeyWords" id="seasonKeyWords" cols="30" rows="5" required class="ui-textarea" placeholder="赛季关键词，用英文逗号间隔，如：迷你世界,建造大赛">${season.seasonKeyWords}</textarea>
			</div>
			<div class="form-item">
			...
			<div class="form-item">
				<button type="submit" class="ui-button ui-button-success" role="button">确认修改</button>
			</div>
		</form>
	</div>
</div>
...
<script>
    ...
    var contributeDates = {
        startTime: ${season.submitStartTime},
        endTime: ${season.submitEndTime}
    };
    ...
    $('#dateRange').dateTime({
        min: getTodayDate(),
		value: dayjs(dateRange.startTime).format('YYYY-MM-DD') + ' 至 ' + dayjs(dateRange.endTime).format('YYYY-MM-DD')
    }).change(function () {
        var result = this.value.split(' ');
        dateRange.startTime = dayjs(result[0]+' 00:00:00').valueOf(); // new Date(result[0]+' 00:00:00').getTime()/1000;
        dateRange.endTime = dayjs(result[2]+' 00:00:00').valueOf(); //new Date(result[2]+' 00:00:00').getTime()/1000;
    });

    $('#contributeDates').dateTime({
        min: getTodayDate(),
        value: dayjs(contributeDates.startTime).format('YYYY-MM-DD') + ' 至 ' + dayjs(contributeDates.endTime).format('YYYY-MM-DD')
    }).change(function () {
        var result = this.value.split(' ');
        contributeDates.startTime = dayjs(result[0]+' 00:00:00').valueOf();
        contributeDates.endTime = dayjs(result[2]+' 00:00:00').valueOf();
    });
    ...
    new Validate($('#updateSeasonForm'), function () {
        ...
        $.ajax({
            url: '<%=path%>/season/updateSeason',
            type: 'POST',
            data: {
                originalSeasonId: '${season.id}',
                updateSeasonId: $('#seasonId').val(),
